...
function viType(type) {
    const map = { LENS: 'TRÒNG KÍNH', FRAME: 'GỌNG KÍNH', SUNGLASSES: 'KÍNH MÁT', CONTACT_LENS: 'CONTACT LENS' };
...
window.addEventListener('click', (event) => {
    const modals = document.querySelectorAll('.modal, .modal-overlay, [id$="Modal"]');
...
function showProductDetails(id) {
    ...
    document.getElementById('modalProductType').innerText = `THƯƠNG HIỆU / LOẠI: ${viType(currentSelectedProduct.type)}`;
    document.getElementById('modalProductDesc').innerText = currentSelectedProduct.description || "Chưa có mô tả cho sản phẩm này.";
...
document.getElementById('confirmAddCartBtn').onclick = async () => {
    ...
    try {
        ...
        alert('Lỗi thêm giỏ hàng! ' + e.message);
    }
}
...
async function loadOrderHistory() {
    ...
    try {
        ...
        orders.forEach(o => {
            ...
            else if (st === 'REFUNDED') { statusText = 'Hoàn Tiền'; color = '#2563eb'; } 
